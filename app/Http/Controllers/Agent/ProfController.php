...
class ProfController extends Controller
{
    public function index(){
        $profagent = profagent::where('agent_id',Auth::guard('agent')->user()->post_id)->first();
        if(!$profagent)
        {
            $profagent = new profagent();
            $profagent->agent_id = Auth::guard('agent')->user()->post_id;
            $profagent->save();   
        }
        $agent=Agent::find(Auth::guard('agent')->user()->post_id);
        return view('agent.profile.index',compact('agent'));

    }
    public function edit($id)
    {
        $agent = Agent::where('agent_id',Auth::guard('agent')->user()->post_id)->first();
